...
abstract class TestCase extends Orchestra
{
    ...
    protected function getEnvironmentSetUp($app): void
    {
        ...
        $app['config']->set('ichava.core.default_set', 'test');
    }

    /**
     * Point the suite at whichever database `DB_CONNECTION` names.
     *
     * The suite ran on SQLite and nothing else, which made every difference between
     * drivers invisible: index-length limits, JSON column semantics, `ON CONFLICT`
     * versus `ON DUPLICATE KEY`, and the PostgreSQL full-text path -- which is the
     * larger half of the search code and had never executed under test. CI now runs
    ...
    protected function configureDatabase($app): void
    {
        ...
        $app['config']->set('database.connections.testing', match ($driver) {
            ...
            default => [
                ...
                // present. Without it the suite never enforced a foreign key, so a cascade
                // that MySQL and PostgreSQL apply was untested on the one driver that ran.
    ...
    protected function configureCache($app): void
    {
        ...
        // serialize()/unserialize(). Tests that depend on that boundary must switch to a
        // serialising store themselves -- see tests/Feature/CachePoisoningTest.php.

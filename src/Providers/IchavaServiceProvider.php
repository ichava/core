...
class IchavaServiceProvider extends PackageServiceProvider
{
    ...
    {
        // The package name 'ichava/ichava' causes the published config to be
        // merged under the nested key 'ichava.ichava'. Flatten back to the
        // top-level 'ichava' key so config('ichava.*') works as documented.
        $this->aliasIchavaConfig();
    }
    ...
    {
        if (! config('ichava.logging.auto_cleanup', true)) {
            return;
        }

        $this->callAfterResolving(Schedule::class, function ($schedule) {
            $cleanupTime = config('ichava.logging.cleanup_time', '03:00');
    ...
    protected function registerFileWatcher(): void
    {
        ...
        $this->callAfterResolving(Schedule::class, function ($schedule) {
            $interval = config('ichava.database.sync_interval', 60);

            $schedule->command('ichava:watch')
                ->everyMinute()
                ->when(fn () => config('ichava.database.auto_sync', true))
                ->runInBackground()
    ...
    {
        $customIconsConfig = config('ichava.custom-icons.sets', []);
    ...
    protected function registerHorizonSupervisor(): void
    {
        ...
        $this->app->booted(function () use ($supervisorConfig, $environment) {
            ...
            if (! config('ichava.queue.horizon.auto_register', true)) {
                return;
            }
    ...
    /**
     * Re-merge the auto-namespaced 'ichava.ichava' config block into the
     * top-level 'ichava' key. Without this every consumer would have to read
     * config('ichava.ichava.logging.*') instead of config('ichava.logging.*').
     */
    protected function aliasIchavaConfig(): void
    {
        if ($config = $this->app['config']->get('ichava.ichava')) {
            $existing = $this->app['config']->get('ichava', []);
            $this->app['config']->set('ichava', array_merge($existing, $config));
        }
    }
    ...
    protected function configureRuntimeSettings(): void
    {
        ...
        $memoryLimit = config('ichava.runtime.memory_limit', '1G');
        $maxExecutionTime = (int) config('ichava.runtime.max_execution_time', 0);
        $disableTelescopeInQueue = config('ichava.runtime.disable_telescope_in_queue', true);

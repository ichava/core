...
final class IchavaLogger
{
    ...
    protected bool $logSecurity;

    protected string $seedingChannel;

    public function __construct()
    {
        // Channels are registered dynamically by IchavaServiceProvider
        // Defaults match the channel names registered there
        $this->channel = config('ichava.logging.channel', 'ichava');
        $this->enabled = config('ichava.logging.enabled', true);
        $this->logPerformance = config('ichava.logging.performance', false);
        $this->logSecurity = config('ichava.logging.security', true);
        $this->seedingChannel = config('ichava.logging.seeding_channel', 'ichava-icons');
    }

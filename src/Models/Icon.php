...
final class Icon extends Model
{
    ...
    public function scopeFuzzySearch(Builder $query, string $search): Builder
    {
         ...
         * `jsonb` -- Laravel's `$table->json()` emits `json` on PostgreSQL -- and there is no
         * implicit cast from `json` to `jsonb`, so `jsonb_array_elements_text(keywords)` does
         * not resolve to any function. The migration's own trigger already had this right
